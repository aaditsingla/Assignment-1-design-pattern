...
/**
 * Concrete implementation of Smart Bulb for Brand A
 */
public class ProductBulbA implements ProductBulb {
    ...
    private void loadPowerUsage() {
        try (BufferedReader reader = new BufferedReader(new FileReader("data/device_usage_data.txt"))) {
            ...
            while ((line = reader.readLine()) != null) {
                if (line.startsWith("BULB_A_POWER:")) {
                    this.powerUsage = Double.parseDouble(line.split(":")[1].trim());
                    break;
                }
            }
        } catch (IOException | NumberFormatException e) {
            System.err.println("Error reading power usage data for Brand A Bulb: " + e.getMessage());
            this.powerUsage = 10.0; // Default value
        }
    }
