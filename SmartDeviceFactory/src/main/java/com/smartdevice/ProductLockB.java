...
/**
 * Concrete implementation of Smart Lock for Brand B
 */
public class ProductLockB implements ProductLock {
    ...
    private void loadBatteryConsumption() {
        try (BufferedReader reader = new BufferedReader(new FileReader("data/device_usage_data.txt"))) {
            ...
            while ((line = reader.readLine()) != null) {
                if (line.startsWith("LOCK_B_BATTERY:")) {
                    this.batteryConsumption = Double.parseDouble(line.split(":")[1].trim());
                    break;
                }
            }
        } catch (IOException | NumberFormatException e) {
            System.err.println("Error reading battery consumption data for Brand B Lock: " + e.getMessage());
            this.batteryConsumption = 7.5; // Default value
        }
    }
